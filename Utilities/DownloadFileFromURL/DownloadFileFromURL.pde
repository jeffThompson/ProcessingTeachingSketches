...
Takes a single argument: a string of what the file should be named when saved.
...
// WHAT TO NAME THE RESULTING FILE
String fileToSaveAs = "index.html";
...
void setup() {
  ...
  try {
    ...
    URL url = new URL("http://www.google.com");    // must be in 'try' statement, since URL throws an exception
    ...
    // saved to 'data' folder so it can be read directly
    FileOutputStream writer = new FileOutputStream(sketchPath + "/data/" + fileToSaveAs);
    ...
    while ((bytesRead = fileDownloader.read(buffer)) > 0) {  // -1 means we're at the end of the file
      ...
      writer.write(buffer, 0, bytesRead);  // write to file
    ...
    println("Finished downloading (" + totalBytesRead + " bytes)");
  }
